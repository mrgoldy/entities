	...
	protected function inject_dbal_driver()
	{
		...
		if (!empty($config_data))
		{
			if ($this->dbal_connection === null)
			{
				...
				# Map to correct driver
				$this->container->set('doctrine', \Doctrine\DBAL\DriverManager::getConnection([
					'dbname'	=> $this->config_php_file->get('dbname'),
					'host'		=> $this->config_php_file->get('dbhost'),
					'post'		=> $this->config_php_file->get('dbport'),
					'user'		=> $this->config_php_file->get('dbuser'),
					'password'	=> $this->config_php_file->get('dbpasswd'),
					'driver'	=> substr($dbal_driver_class, strrpos($dbal_driver_class, "\\") + 1),
				]));
			}
